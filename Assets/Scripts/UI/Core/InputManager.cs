...
namespace NovaSamples.UIControls
{
    ...
    {
        [Header("Allow Close When Click Off")]
        [SerializeField] bool allowCloseWhenClickOff = true;
        [SerializeField] Enum_UIMenuPage uiPageWhenClickOff = Enum_UIMenuPage.Gameplay;

        #region #PUBLIC_API

        /// <summary>
        /// Returns true on the frame a click/tap RELEASED over a Nova UI object,
        /// with the clicked UIBlock in <paramref name="uiBlock"/>.
        /// Returns false when the release wasn't over any Nova UI.
        /// </summary>
        public static bool NovaUIClickedThisFrame(out UIBlock uiBlock)
        {
            // TRUE ONLY ON THE EXACT FRAME WE RECORDED A RELEASE
            if (s_LastClickFrame == Time.frameCount && s_LastClickedUIBlock != null)
            {
                uiBlock = s_LastClickedUIBlock;
                return true;
            }
        ...
        /// <summary>
        /// Returns the most recent Nova UI click (even if not this frame).
        /// </summary>
        public static bool TryGetLatestNovaUIClick(out UIBlock uiBlock)
        {
            uiBlock = s_LastClickedUIBlock;
            return uiBlock != null;
        }
        ...
        #region #STATE
        // LAST NOVA UI CLICK INFO (UPDATED ON MOUSE UP / TOUCH END)
        private static UIBlock s_LastClickedUIBlock;
        private static int s_LastClickFrame = -1;

        private static void RecordClick(UIBlock block)
        {
            s_LastClickedUIBlock = block;
        ...
        private void UpdateMouse()
        {
            ...
            if (LeftMouseButtonUp)
            {
                // QUERY NOVA FOR WHO HANDLED THIS POINTER ID, THEN RECORD IT
                Interaction.TryGetActiveReceiver(MousePointerControlID, out UIBlockHit hit);
                RecordClick(hit.UIBlock);

                // Debug.Log(hit.UIBlock != null
                //     ? $"Mouse click released over: {hit.UIBlock.name}"
                //     : "Mouse click released with no UIBlock under the pointer.");

                if (allowCloseWhenClickOff && hit.UIBlock == null)
                {
                    EventBus<UpdateUIPageEvent>.Raise(new UpdateUIPageEvent() { uiPage = uiPageWhenClickOff });
                }


                // EXISTING SAMPLE EVENT (OPTIONAL)
                //OnPostClick?.Invoke(hit.UIBlock);
            }
        ...
        private void UpdateTouch()
        {
            ...
            for (int i = 0; i < TouchCount; i++)
            {
                ...
                if (!pointerDown)
                {
                    // IMPORTANT: CAPTURE THE ACTIVE RECEIVER *BEFORE* WE CLEAR/CANCEL
                    Interaction.TryGetActiveReceiver(GetTouchID(touch), out UIBlockHit hit);
                    RecordClick(hit.UIBlock);
                    ...
                    // FOLLOW-UP HOVER + CANCEL FOR CONSISTENT EXIT BEHAVIOR
                    update.Ray = default;
                    Interaction.Point(update, false);
                    Interaction.Cancel(update);
                }
